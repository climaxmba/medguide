...
export function LiveAudioInterface() {
    ...
    try {
      ...
      try {
        sessionRef.current.sendClientContent({
          turns:
            "Hello! I am ready for my initial consultation. Please introduce yourself.",
          turnComplete: true,
        });
      } catch (e) {
  ...
  const generateSummaryHandler = async () => {
    ...
    try {
      ...
      let prompt =
        "Summarize the following healthcare consultation between a patient and an AI named Vora. Provide a professional, clinical-style summary highlighting key points, symptoms discussed (if any), and advice given. Keep it concise, using clear bullet points.";
      let transcriptText = finalTranscripts
        .map((t) => `${t.role === "user" ? "Patient" : "Vora"}: ${t.text}`)
        .join("\n\n");
      ...
      const response = await genAI.models.generateContent({
        ...
        config: {
          systemInstruction:
            "You are generating a summary for a voice session.",
        },
      });
      ...
      <main className="flex-1 flex flex-col p-6 bg-slate-50 min-h-screen">
        <div className="max-w-6xl mx-auto w-full bg-white p-6 sm:p-12 rounded-xl">
          <div className="flex justify-between items-center mb-8 print:hidden">
            ...
            {isPrintingSupported && (
              <button
                ...
                className="px-4 py-2 bg-slate-900 text-white rounded-full text-sm font-medium hover:bg-slate-800 transition-colors flex items-center gap-2 shadow-sm"
              >
          ...
          <div className="mb-8">
            ...
            <p className="text-slate-500 mt-2 print:text-slate-700">
            ...
            <div className="flex flex-col items-center justify-center py-20 text-slate-500 print:hidden">
              ...
              {summary === "Failed to generate summary." && (
                <button
                  ...
                  className="mt-4 px-6 py-2 rounded-full bg-red-500 text-white font-medium hover:bg-red-600 transition-colors flex items-center gap-2 shadow-sm"
                >
  ...
  const getSummaryButtonText = () => {
    ...
    } else if (summary === "Failed to generate summary.") {
      return (
        <span className="flex items-center gap-2">
          <Play size={18} className="fill-current text-slate-700" />
          Retry Summary Generation
        </span>
      );
    } else if (summary) {
      ...
      <div className="relative z-10 w-full max-w-lg">
        ...
        <div className="bg-white/80 backdrop-blur-3xl border border-slate-200 shadow-xl p-12 rounded-[3xl] flex flex-col items-center">
          <div className="relative w-48 h-48 mb-12 flex justify-center items-center">
            ...
            <div
              className={`w-32 h-32 rounded-full flex items-center justify-center shadow-xl relative z-10 transition-colors duration-700 ${isActive ? "bg-gradient-to-tr from-red-600 to-red-500 shadow-red-500/40" : isConnecting ? "bg-slate-200" : "bg-slate-100"}`}
            >
              {isConnecting ? (
          ...
          {!isActive && !isConnecting && seconds > 5 && (
            <div className="mt-6 flex flex-col items-center">
              <button
                disabled={(!hasUserSpoken && !summary) || isGeneratingSummary}
                onClick={() => {
              ...
              {!hasUserSpoken && !summary && (
                <span className="text-xs text-slate-400 mt-2">
                  No user speech detected during this session
                </span>
              )}
            </div>
          )}
